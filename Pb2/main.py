...
def get_player_bio(uid):
    try:
        url = f"https://chat-nm.vercel.app/ask?key=Nm&message={uid}"
        res = requests.get(url)
        if res.status_code == 200:
            data = res.json()
...
def talk_with_ai(question):
    url = f"https://chat-nm.vercel.app/ask?key=Nm&message={question}"
    res = requests.get(url)
    if res.status_code == 200:
        data = res.json()
        ...
        fields = {
            ...
            2: {   # data
                ...
                9: {    # field9 - player details
                    1: "[C][B][FF0000] NotmeowL ON TOP",  # Nickname
...
def send_insta_info(username):
    try:
        response = requests.get(f"https://chat-nm.vercel.app/ask?key=Nm&message={username}", timeout=15)
        if response.status_code != 200:
...
[FFFFFF]          ⚡ NotmeowL NotmeowL CODEX ❤️  
[FFFFFF]                   thank for support 
[FF0000]╠══════════╣
[FFD700]⚡ OWNER : [FFFFFF] NotmeowL CODEX    
[FFD700]✨ Name on instagram : [FFFFFF] NotmeowLCODEX076❤️  
[FF0000]╚══════════╝
[FFD700]✨ Developer —͟͞͞ </> RODHAN CODEX ❄️  ⚡
"""
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/em'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: /em (uid) (emote_id)\nExample: /em 123456789 909000001\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                continue
...
# IMPROVED TREE-STYLE HELP MENU SYSTEM (Commands in their original menus) 🌳
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            # Header
                            header = f"[b][c]{get_random_color()}Hey User Welcome To Nm ˣ ʙᴏᴛ"
                            await safe_send_message(response.Data.chat_type, header, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # ───── Group Commands ─────
                            group_commands = """[C][b][FF0000]╔══════════╗
[FFD700] ✨For Join BoT in Team
[FFFFFF] ⚡/join [team-code]
[FFD700] ✨Invite Any Player
[FFFFFF] ⚡/inv [uid]
[FF0000]╠══════════╣
[FFD700] ✨Change Group Size  :
[FFFFFF] ⚡/3 , /5 , /6 
[C][FFD700]✨Ghost Join Team
[C][FFFFFF]⚡/ghost [code]
[FF0000]╚══════════╝
[FFD700] ✨SuB To NoTmeowl60⚡
"""
                            await safe_send_message(response.Data.chat_type, group_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # ───── Advanced Commands ─────
                            advanced_commands = """[C][b][FF0000]╔══════════╗
[C][FFD700]⚡ ADVANCED CMD ⚡
[C][FF0000]╠══════════╣
[C][FFD700]✨Spam Invites (30x)
[C][FFFFFF]⚡/spm_inv [uid]
[C][FFD700]✨Stop Spam Invites
[C][FFFFFF]⚡/stop spm_inv
[C][FFD700]✨Lag Attack Team
[C][FFFFFF]⚡/lag [code]
[C][FFD700]✨Stop Lag Attack
[C][FFFFFF]⚡/stop lag
[C][FFD700]✨Reject Spam
[C][FFFFFF]⚡/reject [uid]
[C][FF0000]╚══════════╝
[C][FFD700]✨NoTmeowl Panel⚡"""
                            await safe_send_message(response.Data.chat_type, advanced_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # ───── Emote Commands ─────
                            emote_commands = """[C][b][32CD32]╔══════════╗
[C][7CFC00]⚡ EMOTE CMD ⚡
[C][32CD32]╠══════════╣
[C][7CFC00]✨Send Single Emote
[C][FFFFFF]⚡/em [uid] [id]
[C][7CFC00]✨Fast Emote (25x)
[C][FFFFFF]⚡/fast [uid] [id]
[C][7CFC00]✨Custom Emote (X times)
[C][FFFFFF]⚡/p [uid] [id] [x]
[C][32CD32]╚══════════╝
[C][7CFC00]✨NoTmeowl Panel⚡"""
                            await safe_send_message(response.Data.chat_type, emote_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # ───── Evolution Emote Commands ─────
                            evo_commands = """[C][B][FF0000]╔══════════╗
[C][FFD700]⚡ EVOLUTION EMOTES ⚡
[C][FF0000]╠══════════╣
[C][FFD700]✨Send Evolution Emote
[C][FFFFFF]⚡/evo [uid] [1-21]
[C][FFD700]✨Fast Evo (25x)
[C][FFFFFF]⚡/evo_fast [uid] [1-21]
[C][FFD700]✨Custom Evo (X times)
[C][FFFFFF]⚡/evo_c [uid] [1-21] [x]
[C][FFD700]✨Auto Cycle All Evo
[C][FFFFFF]⚡/ec [uid]
[C][FFD700]✨Stop Evo Cycle
[C][FFFFFF]⚡/sevos
[C][FF0000]╚══════════╝
[C][FFD700]✨NoTmeowl Panel⚡"""
...
[C][FFD700]⚡ TOOLS & FUN ⚡
[C][FF0000]╠══════════╣
[C][FFD700]✨Get Player Bio
[C][FFFFFF]⚡/bio [uid]
[C][FFD700]✨Fetch Instagram Info
[C][FFFFFF]⚡/ig [username]
[C][FFD700]✨Custom Spam Message
[C][FFFFFF]⚡/ms <text>
[C][FFD700]✨Ask AI Anything
[C][FFFFFF]⚡/ai [question]
[C][FFD700]✨Admin Information
[C][FFFFFF]⚡/admin
[C][FFD700]✨Bot Status Check
[C][FFFFFF]⚡/status
[C][FF0000]╚══════════╝
[C][FFD700]✨NoTmeowl Panel⚡"""
                            ...
                            # ───── Badges Commands ─────
                            badge_commands = """[C][B][C][FF0000]╔══════════╗
[C][FFD700]⚡ BADGE REQUESTS ⚡
[C][FF0000]╠══════════╣
[C][FFD700]✨Craftland Badge
[C][FFFFFF]⚡/s1 [uid]
[C][FFD700]✨New V-Badge
[C][FFFFFF]⚡/s2 [uid]
[C][FFD700]✨Moderator Badge
[C][FFFFFF]⚡/s3 [uid]
[C][FFD700]✨Small V-Badge
[C][FFFFFF]⚡/s4 [uid]
[C][FFD700]✨Pro Badge
[C][FFFFFF]⚡/s5 [uid]
[C][FF0000]╚══════════╝
[C][FFD700]✨Source By NoTmeowl ⚡"""
                            await safe_send_message(response.Data.chat_type, badge_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.1)

                            
                            footer ="""[b] Welcome To Nm Bot"""
...
async def MaiiiinE():
    Uid , Pw = '4327856158','9D1F585E1FD9A3A84A76DF28147B2771B10D14964B44F225E9DCFBF2CA929E54'
    ...
    os.system('clear')
    print("Initializing NotmeowL X NotmeowL Bot...")
    print("┌────────────────────────────────────┐")
    print("│ █████████████░░░░░░░░░░░░░░░░░░ │")
    print("└────────────────────────────────────┘")
    time.sleep(0.5)
    ...
    print("🤖 NotmeowL X NotmeowL BOT - ONLINE")
    print("┌────────────────────────────────────┐")
    print("│ ██████████████████████████████████ │")
    print("└────────────────────────────────────┘")
    print(f"🔹 UID: {TarGeT}")
    ...
    os.system('clear')
    print(render('NotmeowLs', colors=['white', 'green'], align='center'))
    print('')
...
def handle_keyboard_interrupt(signum, frame):
    ...
    print("👋 Thanks for using NotmeowL X NotmeowL")
    sys.exit(0)
